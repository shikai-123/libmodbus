...
/*该函数应将确认请求存储在 响应长度（如果成功）。
返回的请求长度可以为零，如果 指示请求被忽略（例如，对RTU中另一个从站的查询 模式）。
否则，它将返回 -1 并设置 errno。rsp
*/
MODBUS_API int modbus_receive_msgs(modbus_t * ctx, uint8_t * req, uint8_t * rbuf, int * rlen)
{
    ...
    int rc = *rlen =  ctx->backend->receive(ctx, req);
    for (int i = 0; i < rc; i++)
    {
        rbuf[i] = req[i];
    }
...
int modbus_receive_confirmation(modbus_t *ctx, uint8_t *rsp)
{
    ...
    return _modbus_receive_msg(ctx, rsp, MSG_CONFIRMATION, NULL, NULL);
}
/*该函数应将确认请求存储在 响应长度（如果成功）。
返回的请求长度可以为零，如果 指示请求被忽略（例如，对RTU中另一个从站的查询 模式）。
否则，它将返回 -1 并设置 errno。rsp
...
/* Reads IO status  */
static int read_io_status(modbus_t *ctx, int function, int addr, int nb, uint8_t *dest, uint8_t * sbuf, int * slen, uint8_t * rbuf, int * rlen)
{
    int rc;
    int req_length;
